...
class RotaryEmbedding(nn.Module):
    ...
    def compute_default_rope_parameters(self, config=None, device=None, seq_len=None):
        return ROPE_INIT_FUNCTIONS["default"](device=device, base=self.base, dim=self.dim)
...
class Attention(nn.Module):
    ...
    def _build_sdpa_mask(self, seq_len: int, device: torch.device):
        left_window, right_window = self.window_size
        uses_window = left_window >= 0 or right_window >= 0
        if not self.causal and not uses_window:
            return None
        ...
        if left_window >= 0:
            mask &= key_pos >= query_pos - left_window
        if right_window >= 0:
            mask &= key_pos <= query_pos + right_window
    ...
    def _sdpa_varlen_attention(
        ...
        for length in seq_len.tolist():
            ...
            mask = self._build_sdpa_mask(int(length), query.device)
    ...
    def forward(
        ...
        if flash_attn_varlen_func is not None and query_states.is_cuda:
            ...
            attn_output = self._sdpa_varlen_attention(
                query_states,
                key_states,
                value_states,
                seq_len,
...
class MiMoAudioTokenizer(PreTrainedModel):
    ...
    def __init__(self, config: MiMoAudioTokenizerConfig):
        ...
        # Transformers 5.x expects this attribute during from_pretrained finalization.
        # Calling post_init() would also run generic initialization that does not know
        # about this project's custom RoPE module.
