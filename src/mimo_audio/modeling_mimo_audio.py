...
from transformers import GenerationConfig, GenerationMixin, StoppingCriteria
try:
    from transformers import StoppingCriteriaList
except ImportError:
    from transformers.generation.utils import StoppingCriteriaList
...
def _compiler_stance(stance: str):
    compiler = getattr(torch, "compiler", None)
    if compiler is not None and hasattr(compiler, "set_stance"):
        try:
            return compiler.set_stance(stance)
        except Exception:
            return nullcontext()
    return nullcontext()
...
def _sync_cuda_if_needed(device: torch.device | str, enabled: bool) -> None:
    if enabled and torch.device(device).type == "cuda" and torch.cuda.is_available():
        torch.cuda.synchronize(device)


def _new_dynamic_cache(config=None):
    if _transformers_major_version() >= 5 and config is not None:
        try:
...
class MiMoAudioForCausalLM(Qwen2PreTrainedModel, GenerationMixin):
    def __init__(
        ...
        config = (
            MiMoAudioConfig(**config.to_dict())
            if isinstance(config, Qwen2Config) and not isinstance(config, MiMoAudioConfig)
            else config
        )
    ...
    def local_forward(
        ...
        past_key_values = _new_dynamic_cache(self.local_config)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if getattr(self, "_supports_cache_class", True):
    ...
    def _prepare_plain_generation_config(
        self,
        ...
        return generation_config, remaining_kwargs

    def _has_unfinished_sequences_plain(
        self,
    ...
    def _update_model_kwargs_for_plain_generation(
        self,
        outputs: MiMoAudioOutput,
        model_kwargs: dict,
    ):
        ...
        if attention_mask is not None:
            next_mask = attention_mask.new_ones((attention_mask.shape[0], 1))
    ...
    def generate(
        ...
        **kwargs,
        ...
        generation_config, model_kwargs = self._prepare_plain_generation_config(
            generation_config, kwargs
        )

        if inputs is None:
            raise ValueError("`inputs` is required for MiMoAudio generation.")
        input_ids = inputs
        ...
        model_kwargs["past_key_values"] = _new_dynamic_cache(self.config)
    ...
    def slm_sample(
        ...
        **model_kwargs,
        ...
        initial_len = cur_len
        total_new_tokens = max(0, max_length - initial_len)
        ...
        if progress_enabled:
            if _tqdm is not None:
                ...
                print(
                    f"Generate: prompt groups={initial_len}, max groups={max_length}, "
                    f"budget={total_new_tokens}",
                    flush=True,
                )
                ...
                progress_callback(
                    {
                        ...
                        "cuda_memory": _cuda_memory_text(input_ids.device),
                    }
                )

        try:
            while self._has_unfinished_sequences_plain(
                this_peer_finished,
                synced_gpus,
                device=input_ids.device,
                cur_len=cur_len,
                ...
                _sync_cuda_if_needed(input_ids.device, progress_enabled)
                ...
                if is_prefill:
                    with _compiler_stance("force_eager"):
                ...
                token_id = int(next_text_tokens[0].detach().cpu())
                if next_text_tokens[0] != self.args.empty_idx:
                    ...
                    _sync_cuda_if_needed(input_ids.device, progress_enabled)
                    local_seconds = time.monotonic() - local_start
                    local_mode = "speech"
                ...
                model_kwargs = self._update_model_kwargs_for_plain_generation(
                    outputs,
                    model_kwargs,
                )
                ...
                this_peer_finished = bool((unfinished_sequences.max() == 0).item())
                cur_len += 1

                _sync_cuda_if_needed(input_ids.device, progress_enabled)
                step_seconds = time.monotonic() - step_start
                generated = cur_len - initial_len
                now = time.monotonic()
                should_report = (
                    generated == 1
                    or this_peer_finished
                    or now - last_detail_time >= progress_interval
                )

                callback_text = None
                if progress_callback is not None:
                    callback_text = progress_callback(
                        {
                            "event": "step",
                            "generated": generated,
                            ...
                            "finished": bool(this_peer_finished),
                        }
                    )
                ...
                if progress_enabled:
                    if progress_bar is not None:
                        ...
                        if should_report:
                            postfix = (
                                callback_text
                                or f"{local_mode}, {step_seconds:.2f}s, id={token_id}"
                            )
                            progress_bar.set_postfix_str(str(postfix)[:160])
                    elif should_report:
                        detail = callback_text or f"{local_mode}, id={token_id}"
                        print(
                            f"Generate: {generated}/{total_new_tokens} "
                            f"({step_seconds:.2f}s/step, local {local_seconds:.2f}s) "
                            f"{detail} {_cuda_memory_text(input_ids.device)}",
                            flush=True,
                        )
                    if should_report:
